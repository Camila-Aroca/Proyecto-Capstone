...
### Sincronización del modelo de datos y UML de clases

- Esta regla aplica únicamente a `docs/07-diagramas/15-modelo-base-datos/README.md`, `modelo_datos_logico.mmd`, `modelo_datos_logico.svg`, `modelo_datos_logico.png`, `modelo_datos_fisico_postgresql.mmd`, `modelo_datos_fisico_postgresql.svg`, `modelo_datos_fisico_postgresql.png`, `db/schema_postgresql.sql`, `docs/07-diagramas/clases_saad.puml`, `docs/07-diagramas/clases_saad.svg` y `docs/07-diagramas/clases_saad.png`; no se extiende automáticamente a otros UML.
- Evaluar esta sincronización cuando una tarea produzca un cambio material del modelo canónico: creación o eliminación de dimensión, fact, mart o dataset analítico relevante; cambio de grano, PK/FK, cardinalidad, relación, tipo, semántica o atributo representado; cambio de persistencia PostgreSQL/PostGIS; o nueva entidad relevante del MVP.
- Ante un cambio material, evaluar explícitamente los artefactos afectados y actualizarlos dentro de la misma tarea, modificando solo los que realmente correspondan. No activar la regla por nuevos valores/filas, regeneración con el mismo schema/grain, cifras, EDA, tests, documentación no estructural, refactors, optimizaciones ni cambios que preserven exactamente el contrato.
- El ERD lógico representa entidades/datasets, grano, claves lógicas, relaciones y cardinalidades; el ERD físico representa la persistencia PostgreSQL/PostGIS, tablas y PK/FK; `db/schema_postgresql.sql` implementa el diseño físico y debe ser coherente con el ERD físico; el UML representa clases conceptuales, atributos principales, operaciones y asociaciones, sin convertirse automáticamente en copia del ERD. Si cambia el DDL, comprobar el ERD físico, y viceversa; si cambia una clase, atributo, operación o asociación, actualizar el UML y sus salidas.
- Al cambiar `modelo_datos_logico.mmd`, regenerar desde esa fuente `modelo_datos_logico.svg` y `modelo_datos_logico.png`. Al cambiar `modelo_datos_fisico_postgresql.mmd`, regenerar sus SVG/PNG. Al cambiar `clases_saad.puml`, regenerar `clases_saad.svg` y `clases_saad.png`. Las salidas SVG/PNG siempre deben generarse desde su fuente, nunca editarse manualmente.
- Tras regenerar, validar que la fuente renderiza sin error, que SVG/PNG existen y no están vacíos, y que representan la versión actual de la fuente. Preservar siempre estas invariantes: Urgencias se relaciona con establecimientos mediante `establecimiento_codigo`; Egresos no identifica el establecimiento hospitalario y su territorio representa residencia cuando corresponda; no hay unión por paciente entre Urgencias y Egresos; ID36 no equivale estrictamente a F00-F99; y Egresos representa registros publicados, no pacientes únicos.
- No realizar reauditorías completas: usar el diff, el contrato modificado y la evidencia upstream de la tarea. Antes de cerrar un cambio material, indicar qué artefactos se evaluaron, cuáles se actualizaron y cuáles no requerían cambio, brevemente.
