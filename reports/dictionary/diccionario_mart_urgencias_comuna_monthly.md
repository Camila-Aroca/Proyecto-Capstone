# Diccionario de datos: `mart_urgencias_comuna_monthly`

- **Ruta:** `data/processed/marts/mart_urgencias_comuna_monthly.parquet`
- **Constructor:** `src/data/build_urgencias_comuna_marts.py` (`build_monthly_mart`), stage `build_urgencias_comuna_marts` en `PIPELINE.md`.
- **Grano:** una fila = `comuna_codigo` × `ano` × `mes` (mes calendario derivado de la `fecha` diaria del origen, no del calendario semanal DEIS).
- **Cobertura observada:** 51 de las 52 comunas RM y 60 meses por comuna (2021 a 2025, 12 meses/año), 3.060 filas en total. Falta la comuna Vitacura (`13132`); ver limitaciones.
- **Fuente:**
...
- **Construcción/derivación:** filtra únicamente `id_causa ∈ {1, 35, 36, 37, 38, 39, 40, 41}`; agrega, por comuna×mes×causa, la suma de `total` (atenciones) y el conteo de establecimientos distintos que reportaron (`nunique` de `establecimiento_codigo`); pivotea a columnas por causa; calcula proporciones internas y tasas por 10.000 habitantes uniendo la población anual (la misma cifra de población se reutiliza para los 12 meses del año, sin interpolar mensual). No genera meses ausentes ni sustituye la ausencia de una causa por cero.
...
| `fecha_mes` | string (fecha ISO `YYYY-MM-DD`) | Primer día del mes calendario (`ano`-`mes`-01), derivado de `fecha` diaria del origen. |
...
| `n_establecimientos_reportantes_id1` | int64 | Cantidad de establecimientos distintos que reportaron al menos una atención ID1 en esa comuna×mes. |
| `n_establecimientos_reportantes_id36` | int64 | Cantidad de establecimientos distintos que reportaron al menos una atención ID36 en esa comuna×mes. |
...
| `poblacion_anual` | int64 | Población anual de la comuna (INE, proyecciones), igual para los 12 meses del año. |
...
- **Las tasas son intensidad territorial de atenciones (evento), no prevalencia/incidencia ni personas únicas atendidas.** Una misma persona puede generar varias atenciones en la misma comuna×mes; la tasa no debe leerse como "% de población afectada".
- Los ratios (`proporcion_*`) quedan explícitamente nulos cuando su denominador es 0 o está ausente; nunca se sustituyen por 0.
- `n_establecimientos_reportantes_id1` y `n_establecimientos_reportantes_id36` son idénticos en el 100% de las filas observadas (ver EDA): los establecimientos que reportan ID1 son los mismos que reportan ID36 en la misma comuna×mes.
...
- `mes`/`fecha_mes` son mes calendario estándar (no calendario DEIS); por eso este mart no reconcilia fila a fila con el semanal, pero sí reconcilia exactamente al agregar por comuna×año (ver EDA), ya que ambos comparten fuente y filtros de causa.
