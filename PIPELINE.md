...
21. `build_urgencias_establecimiento_mart` → (Mart histórico establecimiento×mes de Urgencias, 2021–2025, sin población ni tasas)
22. `build_urgencias_comuna_etario_mart` → (Mart histórico comuna×mes×grupo_etario de Urgencias, 2021–2025, formato largo)
23. `build_dim_oferta_urgencia_rm` → (Dimensión canónica de oferta territorial de urgencia RM: snapshot actual, no serie histórica)
24. `build_mart_mvp_territorial_comuna` → (Mart canónico MVP territorial comunal: integra demanda 2025, población 2025/2024, vulnerabilidad 2022 y oferta actual en una fila por comuna RM)
25. `eda_contexto_genero` → (EDA reproducible de los cuatro cuadros contextuales)
